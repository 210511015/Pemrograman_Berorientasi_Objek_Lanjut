...
import json
from tkinter import Frame, Label, Entry, Button, Radiobutton, ttk, VERTICAL, YES, BOTH, END, Tk, W, StringVar, messagebox, Menu, Canvas
from Users import *
from tkinter import *
...
class Dashboard:
    ...
    def menuAdmin(self):
        ...
        admin_menu.add_command(
            label='Kategori', command=lambda: self.new_window("Kategori", FormKategori)
        )

        admin_menu.add_command(
            label='Buku', command=lambda: self.new_window("Buku", FormBuku)
        )
        admin_menu.add_command(
            label='Anggota', command=lambda: self.new_window("Anggota", FormAnggota)
        )
        admin_menu.add_command(
            label='Peminjaman', command=lambda: self.new_window("Peminjaman", FormPeminjaman)
        )
    ...
    def menuAnggota(self):
        ...
        anggota_menu.add_command(
            label='Data anggota 1', command=lambda: self.new_window("Menu anggota 1", FormAnggota)
        )
        anggota_menu.add_command(
            label='Buku', command=lambda: self.new_window("Buku", FormBuku))
...
if __name__ == '__main__':
    ...
    # canvas1.create_text(400, 350, text="Welcome",
    #                     activefill="blue", font=('Cooper_Black 20 bold'))
